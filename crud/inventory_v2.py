...
def get_inventory_report(
    ...
    committed_sq = db.query(models.LineItem.sku, func.sum(models.LineItem.quantity).label("committed")).join(models.Order).filter(models.Order.fulfillment_status.in_(['unfulfilled', 'partially_fulfilled', 'scheduled']), models.Order.cancelled_at.is_(None)).group_by(models.LineItem.sku).subquery('committed_sq')
    base_query = db.query(models.ProductVariant, models.Product, models.Store.name.label("store_name"), func.coalesce(committed_sq.c.committed, 0).label("committed_qty")).join(models.Product, models.ProductVariant.product_id == models.Product.id).join(models.Store, models.Product.store_id == models.Store.id).outerjoin(committed_sq, models.ProductVariant.sku == committed_sq.c.sku)

    if store_ids: base_query = base_query.filter(models.Product.store_id.in_(store_ids))
    if search: base_query = base_query.filter(or_(models.ProductVariant.sku.ilike(f"%{search}%"), models.ProductVariant.barcode.ilike(f"%{search}%"), models.Product.title.ilike(f"%{search}%")))
    if product_type: base_query = base_query.filter(models.Product.product_type == product_type)
    if category: base_query = base_query.filter(models.Product.product_category == category)
    if status: base_query = base_query.filter(models.Product.status == status)
    ...
    aggregates = base_query.with_entities(func.sum(retail_value_col).label("total_retail_value"), func.sum(inventory_value_col).label("total_inventory_value"), func.sum(on_hand_col).label("total_on_hand")).one()
    ...
    if view == 'grouped':
        ...
        # Subquery 1: Aggregate quantitative data and variant details, grouped STRICTLY by barcode
        ...
        # Subquery 2: Identify the single primary variant for each barcode
        ...
        # Subquery 3: Get the display details for that single primary variant
        ...
        # Final Query: Join the aggregated data with the primary variant details
        ...
        results = final_query.order_by(order_func.nulls_last()).offset(skip).limit(limit).all()
