...
def create_or_update_order_from_webhook(db: Session, store_id: int, order_data: schemas.ShopifyOrderWebhook):
    ...
    """
    # ... (existing code for upserting order and line items) ...
    ...
    order_dict = {
        "id": order_data.id, "shopify_gid": order_data.admin_graphql_api_id, "store_id": store_id, "name": order_data.name,
        "email": order_data.email, "phone": order_data.phone, "created_at": order_data.created_at, "updated_at": order_data.updated_at,
        "cancelled_at": order_data.cancelled_at, "cancel_reason": order_data.cancel_reason, "closed_at": order_data.closed_at,
        "processed_at": order_data.processed_at, "financial_status": financial_status, "fulfillment_status": fulfillment_status,
        "currency": order_data.currency, "payment_gateway_names": ", ".join(order_data.payment_gateway_names) if order_data.payment_gateway_names else None,
        "note": order_data.note, "tags": order_data.tags, "total_price": order_data.total_price, "subtotal_price": order_data.subtotal_price,
        "total_tax": order_data.total_tax, "total_discounts": order_data.total_discounts, "total_shipping_price": order_data.total_shipping_price_set['shop_money']['amount']
    }
    ...
    for item in order_data.line_items:
        line_items_list.append({
            "id": item.id, "order_id": order_data.id, "variant_id": item.variant_id, "product_id": item.product_id,
            "title": item.title, "quantity": item.quantity, "sku": item.sku, "vendor": item.vendor, "price": item.price,
            "total_discount": item.total_discount, "taxable": item.taxable, "shopify_gid": f"gid://shopify/LineItem/{item.id}"
        })
...
def create_refund_from_webhook(db: Session, store_id: int, refund_data: schemas.ShopifyRefundWebhook):
    """
    Creates refund records from a webhook payload. If the parent order or a
    line item does not exist, placeholders are created to prevent foreign key violations.
    """
    ...
    for item in refund_data.refund_line_items:
        if item.line_item_id not in existing_line_item_ids:
            ...
            line_items_to_create.append({
                ...
                "title": line_item_data.get('title', 'N/A'),
                "quantity": line_item_data.get('quantity', 0),
                "sku": line_item_data.get('sku'),
                "shopify_gid": f"gid://shopify/LineItem/{line_item_data['id']}"
            })
...
def create_or_update_orders(db: Session, orders_data: List[schemas.ShopifyOrder], store_id: int):
    ...
    for order in orders_data:
        ...
        for item in order.line_items:
            # ... (rest of the inner loop is the same) ...
            ...
            if item.variant:
                # ... (product, variant, location processing is the same) ...
        ...
        for fulfillment in order.fulfillments:
            # ... (fulfillment processing is the same) ...
