...
def _parse_store_ids(store_ids: Optional[List[int]]) -> Optional[List[int]]:
    if not store_ids:
        return None
    # Remove falsy and duplicates while preserving numeric type
...
def _base_join(db: Session):
    ...
    Tables used:
      ...
      - GroupMembership (GM)  [optional; variants without a group still count]
      - InventoryLevel (IL)
      - Location (Loc)
      - Product (Product)
    """
...
def _filters_where(filters: Dict[str, Any], Loc, Variant) -> List[Any]:
    ...
    Currently supports store_ids.
    """
    clauses: List[Any] = []

    # store_ids filter: use the store attached to Location (inventory lives at a location)
    store_ids = _parse_store_ids(filters.get("store_ids"))
    if store_ids:
        clauses.append(Loc.store_id.in_(store_ids))

    # tracked-only filter? (kept false by default; uncomment to enforce)
    # clauses.append(or_(Variant.tracked.is_(True), Variant.tracked.is_(None)))
...
def _group_key_expr(GM, Variant):
    """
    Barcode group key. For variants without a barcode/group, we isolate them
    as unique groups ('VARIANT:{id}') to avoid accidental cross-counting.
    """
    return func.coalesce(GM.group_id, func.concat(literal("VARIANT:"), Variant.id))
...
def _build_group_aggregate_query(db: Session, filters: Dict[str, Any]):
    ...
    Builds a grouped-by-barcode query that aggregates:
      ...
      - a 'sample' product & variant title/sku/barcode (for display when needed)
    """
    ...
    grouped = (
        db.query(
            ...
            func.max(Variant.cost_per_item).label("max_cost"),
    ...
    # Re-apply filters (SQLAlchemy builds a new query above)
    ...
    if (filters or {}).get("search"):
        ...
        _apply_search(filters, search_clauses, models.Product, models.ProductVariant, models.GroupMembership)
        where_clauses = where_clauses + search_clauses
...
def get_inventory_report(
    ...
    *,
    ...
    - totals: ALWAYS deduped by barcode group (unless totals_mode='raw', which you likely don't want)
    - total_count: the count for the chosen view (for pagination)
    """
    filters = {
        "search": search,
        "store_ids": store_ids,
    ...
    if (view or "").lower() == "grouped":
        ...
        q = db.query(*columns.values()).select_from(g_subq)
        ...
        for rec in q.all():
            rows.append({
                ...
                "sku": rec[3],
                "barcode": rec[4],
                "on_hand": int(rec[5] or 0),
                "available": int(rec[6] or 0),
                "max_cost": float(rec[7] or 0.0),
                "max_price": float(rec[8] or 0.0),
                "inventory_value": float(rec[9] or 0.0),
                "retail_value": float(rec[10] or 0.0),
            })
        ...
        columns = {
            "variant_id": literal(0),  # not used for sort mapping
            "group_id": literal(0),    # not used for sort mapping
            "product_title": literal(0),
            "variant_title": literal(0),
            "sku": literal(0),
            "barcode": literal(0),
            "on_hand": vq.c.on_hand,
            "available": vq.c.available,
            "cost_per_item": vq.c.cost_per_item,
            "price": vq.c.price,
            "inventory_value": vq.c.on_hand * vq.c.cost_per_item,
            "retail_value": vq.c.on_hand * vq.c.price,
        }

        # count variants
        count_q = db.query(func.count()).select_from(vq.subquery())
        total_count = count_q.scalar() or 0

        # sorting
        sort_expr = _map_sort(sort_by, sort_order, columns)
        vq = vq.order_by(sort_expr)
        ...
        for rec in vq.all():
            rows.append({
                ...
                "inventory_value": float((rec.on_hand or 0) * float(rec.cost_per_item or 0.0)),
                "retail_value": float((rec.on_hand or 0) * float(rec.price or 0.0)),
            })
