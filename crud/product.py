...
def log_dead_letter(db: Session, store_id: int, run_id: int, payload: Dict, reason: str):
    try:
        ...
        dead_letter = models.SyncDeadLetter(store_id=store_id, run_id=run_id, payload=payload_json, reason=reason)
        db.add(dead_letter)
        db.commit()
...
def _extract_product_fields(p_data: Any, store_id: int, last_seen_at: datetime) -> Dict:
    ...
    return { "id": pid, "store_id": store_id, "shopify_gid": p_data.get("id"), "title": p_data.get("title"), "body_html": p_data.get("bodyHtml"), "vendor": p_data.get("vendor"), "product_type": p_data.get("productType"), "status": p_data.get("status"), "handle": p_data.get("handle"), "tags": ",".join(tags if tags is not None else []), "image_url": _first_image_url(p_data), "created_at": _to_dt(p_data.get("createdAt")), "updated_at": _to_dt(p_data.get("updatedAt")), "published_at": _to_dt(p_data.get("publishedAt")), "last_seen_at": last_seen_at }
...
def _extract_variant_fields(v_data: Any, product_id: int, store_id: int, last_seen_at: datetime) -> Dict:
    ...
    # The inventory_item_id was being read from the wrong level of the JSON payload.
    # It needs to be read from the `inventoryItem` object within the variant.
    ...
    return {
        ...
        "inventory_item_id": inventory_item_id, # Use the corrected ID
        "inventory_quantity": v_data.get("inventoryQuantity"),
        "inventory_policy": v_data.get("inventoryPolicy"), "position": v_data.get("position"),
...
def create_or_update_products(db: Session, store_id: int, run_id: int, items: List[Any], last_seen_at: datetime):
    # This function is correct and remains unchanged, as the fix is in _extract_variant_fields
    if not items: return
    ...
    for bundle in items:
        try:
            ...
            product_stmt = pg_insert(models.Product).values(p_row).on_conflict_do_update(index_elements=['id'], set_={k: getattr(pg_insert(models.Product).excluded, k) for k in p_row if k not in ['id', 'store_id']})
            db.execute(product_stmt)
            v_data_list = p_data.get("variants", []) or _get(p_data, "variants", "edges", default=[])
            if v_data_list:
                loc_rows_map = {}
                inv_level_rows = []
                for v_edge in v_data_list:
                    v_data = v_edge.get("node", v_edge)
                    v_row = _extract_variant_fields(v_data, p_row["id"], store_id, last_seen_at)
                    variant_stmt = pg_insert(models.ProductVariant).values(v_row).on_conflict_do_update(index_elements=['id'], set_={k: getattr(pg_insert(models.ProductVariant).excluded, k) for k in v_row if k != 'id'})
                    db.execute(variant_stmt)
                    for lvl in _get(v_data, "inventoryItem", "inventoryLevels", "edges", default=[]):
                        lvl_node = lvl.get("node", lvl)
                        loc_gid = _get(lvl_node, "location", "id")
                        loc_id = gid_to_id(loc_gid)
                        if not loc_id or not loc_gid: continue
                        loc_rows_map[loc_id] = { "id": loc_id, "shopify_gid": loc_gid, "store_id": store_id, "name": _get(lvl_node, "location", "name") }
                        qmap = {q["name"]: q["quantity"] for q in _get(lvl_node, "quantities", default=[])}
                        inv_level_rows.append({ "variant_id": v_row["id"], "location_id": loc_id, "inventory_item_id": v_row["inventory_item_id"], "available": qmap.get("available", 0), "on_hand": qmap.get("on_hand", qmap.get("available", 0)), "last_fetched_at": now, })
                if loc_rows_map:
                    loc_rows = list(loc_rows_map.values())
                    loc_stmt = pg_insert(models.Location).values(loc_rows).on_conflict_do_update(index_elements=['id'], set_={"name": pg_insert(models.Location).excluded.name, "shopify_gid": pg_insert(models.Location).excluded.shopify_gid})
                    db.execute(loc_stmt)
                if inv_level_rows:
                    inv_stmt = pg_insert(models.InventoryLevel).values(inv_level_rows).on_conflict_do_update(index_elements=['variant_id', 'location_id'], set_={"available": pg_insert(models.InventoryLevel).excluded.available, "on_hand": pg_insert(models.InventoryLevel).excluded.on_hand, "last_fetched_at": pg_insert(models.InventoryLevel).excluded.last_fetched_at})
                    db.execute(inv_stmt)
            db.commit()
