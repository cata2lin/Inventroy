...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchAndRender = debounce(async () => {
        ...
        // --- FIXED: Robustly handle adding filter parameters ---
        Object.entries(state.filters).forEach(([key, value]) => {
            // Only append the parameter if it has a meaningful value
            if (value !== null && value !== undefined && value !== '') {
                if (Array.isArray(value)) {
                    // For arrays, only append if it's not empty
                    if (value.length > 0) {
    ...
    ['search', 'tags', 'startDate', 'endDate'].forEach(key => {
        elements.filters[key].addEventListener('input', () => {
            const filterKey = key === 'startDate' ? 'start_date' : (key === 'endDate' ? 'end_date' : key);
    ...
    ['financialStatus', 'fulfillmentStatus', 'hasNote'].forEach(key => {
        elements.filters[key].addEventListener('change', (e) => {
            const filterKey = key === 'financialStatus' ? 'financial_status' : (key === 'fulfillmentStatus' ? 'fulfillment_status' : 'has_note');
    ...
    elements.filters.columns.addEventListener('change', () => {
        ...
        fetchAndRender(); // Only needs to re-render, not re-fetch
    });
