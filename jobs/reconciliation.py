...
def run_reconciliation(db_factory, task_id: Optional[str] = None):
    """
    ...
    try:
        if task_id:
            sync_tracker.step(task_id, 0, "Fetching active barcode groups...")
        ...
        if task_id:
            sync_tracker.step(task_id, 0, f"Found {total_groups} groups to reconcile.")

        for group in groups:
            planned = []  # [(member_variant, target)]
            try:
                with db.begin_nested(): # Use nested transaction for each group
        ...
        if task_id:
            sync_tracker.finish_task(task_id, True, f"Reconciliation complete. Processed {total_groups} groups.")

    except Exception as e:
        if task_id:
            sync_tracker.finish_task(task_id, False, f"Reconciliation failed: {e}")
    finally:
        db.close()
